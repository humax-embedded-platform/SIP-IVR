...
void RequestsHandler::OnRefer(std::shared_ptr<SipMessage> data)
{
	...
	std::string callId = IDGen::GenerateID(24);
	auto newSession = std::make_shared<Session>(callId, caller.value(), session->get()->getSrcRtpPort());
	_sessions.emplace(callId, newSession);

	refer->setHeader(SipMessageTypes::INVITE);
	refer->setCallID("Call-ID: " + callId);
	refer->setContact("Contact: <sip:" + session->get()->getSrc()->getNumber() + "@" + _serverIp + ":" + std::to_string(_serverPort) + ";transport=UDP>");
	refer->setFrom("From: <sip:" + session->get()->getSrc()->getNumber() + "@" + _serverIp + ":" + std::to_string(_serverPort) + ">;tag=" + IDGen::GenerateID(9));
	refer->setTo("To: <sip:" + data->getReferToNumber() + "@" + _serverIp + ":" + std::to_string(_serverPort) + ">");
	refer->setCSeq("CSeq: 1 INVITE");
	refer->setReplaces("Replaces: " + orgCallId);
	endHandle("phong2", refer);
}
...
{
	// std::cout << "[" << __FUNCTION__ << "] Sending to " << destNumber << " with message: " << message->toString() << std::endl;
